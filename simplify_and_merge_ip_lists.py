...
DATA_FILE = FILE_DIR / "cloud_ips.json"

# def _ip_to_bloom(ip):
#     octets = ip.split(".")
#     for n, octet in enumerate(octets):
#         value = 256**(4-n) * int(octet)
...
_leading_ones_lookup = {
    n: _int_to_num_leading_ones(n)
    for n in range(256)
}

_ending_ones_lookup = {
    n: _int_to_num_ending_ones(n)
    for n in range(256)
}

_n_leading_ones_to_int = {
    n: int('0b' + '1' * n + '0'*(8-n), 2)
    for n in range(0,9)
}
...
node_names = iter(_name_generator())
...
def _pop_and_merge(stack: typing.List[IP], merge_with: IP):
    ...
    for i, other_ip in enumerate(reversed(stack)):
        if not any([other_ip.has_overlap_with(merge_ip) for merge_ip in (ip_ranges_to_remerge + [merge_with])]):
            break
...
def _walk_in_order(node: TreeNode, action: typing.Optional[typing.Callable[[TreeNode], None]]=None):
    if node.left:
...
def main(files: typing.List[pathlib.Path], print_graph=False, output: typing.Optional[pathlib.Path]=None, pretty:bool=False):
    st = SBBST()
    ...
    for file in files:
        ...
        while merge_adjacent_in_tree(st):
            pass
...
def _main(files: typing.List[pathlib.Path], print_graph: bool, output: pathlib.Path, pretty: bool):
    main(files, print_graph, output=output, pretty=pretty)
